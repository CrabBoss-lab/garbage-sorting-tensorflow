...
base_path = "./dataset"
# 随机查看一张图
img_list = glob.glob(os.path.join(base_path, "*/*.jpg"))    # 获取指定路径下的所有图片
...
train_data = tf.keras.preprocessing.image.ImageDataGenerator(
    ...
    zoom_range=0.1,     # 随机缩放
    width_shift_range=0.1,  # 在垂直或水平方向上随机平移图片的范围
    height_shift_range=0.1,
    horizontal_flip=True,   # 进行随机水平翻转
    vertical_flip=True,     # 进行随机竖直翻转
    validation_split=0.1    # 分割数据作为验证数据
)
...
train_generator = train_data.flow_from_directory(
    ...
    shuffle=True,   # 随机播放-打乱数据，默认是True
